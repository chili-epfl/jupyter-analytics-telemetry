...
export class WebsocketManager {
  ...
  private _createSocket(notebookId: string, userId: string) {
    ...
    this._socket.on(
      'chat',
      (
        data: { message: string; sender: string; sender_type?: string } | string
      ) => {
        if (this._messageCallback) {
          ...
          if (typeof data === 'string') {
            ...
            const senderType: 'teacher' | 'teammate' =
              data.sender_type === 'teacher' ? 'teacher' : 'teammate';
    ...
    this._socket.on(
      ...
      (data: { userId: string; cellId: string; cellIndex?: number }) => {
        if (this._locationUpdateCallback) {
          ...
          console.log(
            `${APP_ID}: WARNING - No location update callback registered!`
          );
        }
      }
    );

    this._socket.on('teammate_location_cleared', (data: { userId: string }) => {
  ...
  public sendLocationUpdate(cellId: string, cellIndex?: number) {
    if (this._socket) {
      ...
      console.log(
        `${APP_ID}: Cannot send location update - no socket connection`
      );
    }
  }
