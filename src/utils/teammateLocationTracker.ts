...
export class TeammateLocationSidebar extends Widget {
    ...
    constructor() {
        ...
        const refreshBtn = this._headerDiv.querySelector('.teammate-location-refresh-btn');
        if (refreshBtn) {
            refreshBtn.addEventListener('click', () => {
                console.log(`${APP_ID}: Refresh button clicked`);
                this.refresh();
            });
        }
    ...
    public setFetchTeammateLocationsCallback(
        ...
        console.log(`${APP_ID}: Fetch callback set on TeammateLocationSidebar`);
    ...
    public setNotebookPanel(panel: NotebookPanel | null) {
        console.log(`${APP_ID}: setNotebookPanel called`, panel ? 'with panel' : 'null');
        this._notebookPanel = panel;
        if (panel) {
            this._generateHeadings();
            // Wait for panel context to be ready before fetching
            void panel.context.ready.then(() => {
                console.log(`${APP_ID}: Panel context ready, fetching data`);
    ...
    private async _fetchConnectedTeammates() {
        ...
        try {
            ...
            console.log(`${APP_ID}: Connected teammates:`, teammates);
    ...
    private async _fetchInitialLocations() {
        console.log(`${APP_ID}: _fetchInitialLocations called`);
        if (this._fetchTeammateLocationsCallback && this._notebookPanel) {
            const notebookId = CompatibilityManager.getMetadataComp(
                this._notebookPanel.context.model,
                Selectors.notebookId
            );
            console.log(`${APP_ID}: Notebook ID for fetch:`, notebookId);
            if (!notebookId) {
            ...
            try {
                console.log(`${APP_ID}: Calling fetch callback...`);
                const locations = await this._fetchTeammateLocationsCallback(notebookId);
                console.log(`${APP_ID}: Received locations:`, locations);
                this._teammateLocations.clear();
                ...
                console.log(`${APP_ID}: Teammate locations map size:`, this._teammateLocations.size);
                this._render();
            } catch (error) {
    ...
    public updateTeammateLocation(location: ITeammateLocation) {
        console.log(`${APP_ID}: updateTeammateLocation called:`, location);
    ...
    public removeTeammateLocation(userId: string) {
        console.log(`${APP_ID}: removeTeammateLocation called:`, userId);
        this._teammateLocations.delete(userId);
        this._connectedTeammates = this._connectedTeammates.filter(id => id !== userId);
        this._render();
    }
    ...
    public async refresh() {
        console.log(`${APP_ID}: TeammateLocationSidebar refresh called`);
        if (this._notebookPanel) {
            this._generateHeadings();
